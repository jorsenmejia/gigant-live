...
class WCFMmp_Vendor {
	...
	public function is_store_lists_sidebar() {
		...
		$wcfmmp_marketplace_options   = get_option( 'wcfm_marketplace_options', array() );
		$store_list_sidebar = isset( $wcfmmp_marketplace_options['store_list_sidebar'] ) ? $wcfmmp_marketplace_options['store_list_sidebar'] : 'yes';
	...
	function wcfmmp_store_setup_on_first_login() {
		...
		if( is_user_logged_in() && wcfm_is_vendor() && $user_id ) {
			...
			if( !$store_setup ) {
				...
				if( apply_filters( 'wcfm_is_allow_store_setup', true ) && ( $vendor_sold_by == 'yes' ) && ( $disable_wcfm_store_setup == 'no' ) ) {
