		...
		init: function( cart ) {
			...
			$cart
				...
				.on( 'woocommerce-product-addons-update', function() {
					...
					// Appointments compat.
					if ( $( '.wc-appointments-appointment-cost' ).length ) {
						//product_price = parseFloat( appointment_cost ) : ( $( '.wc-appointments-appointment-cost' ).attr( 'data-raw-price' ) );
					}
					...
					$cart.find( '.wc-pao-addon-field' ).each( function() {
						...
						if ( $addon.is( '.wc-pao-addon-custom-price' ) ) {
							...
							addon_duration_raw = $addon.data( 'raw-duration' ) * $addon.val();
							...
							duration_type = $addon.find( 'option:selected' ).data( 'duration-type' );

							if ( $addon.val() ) {
								value_label        = $addon.find( 'option:selected' ).data( 'label' );
								addon_cost         = $addon.find( 'option:selected' ).data( 'price' );
								addon_cost_raw     = $addon.find( 'option:selected' ).data( 'raw-price' );
								addon_duration     = $addon.find( 'option:selected' ).data( 'duration' );
								addon_duration_raw = $addon.find( 'option:selected' ).data( 'raw-duration' );
							}
						} else {
							if ( ! $addon.val() ) {
								return;
						...
						switch ( duration_type ) {
							case 'flat_time':
							case '':
								addon_data.duration = addon_duration_raw;
								addon_data.duration_raw = addon_duration_raw;
								break;
							case 'quantity_based':
								addon_data.duration_pu = addon_duration_raw;
								addon_data.duration_raw_pu = addon_duration_raw;
								addon_data.duration = addon_data.duration_pu * qty;
								addon_data.duration_raw = addon_data.duration_raw_pu * qty;
								break;
						}
					...
					if ( qty ) {
						...
						if ( 'undefined' !== typeof product_price && product_id ) {
							...
							} else if ( $( '.wc-appointments-appointment-cost' ).length ) {
								product_total_price = parseFloat( product_price );
							} else {
