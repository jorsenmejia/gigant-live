...
class Product_Addon_Admin {
	...
	public function admin_menu() {
		$page = add_submenu_page( 'edit.php?post_type=product', __( 'Global Add-ons', 'woocommerce-appointments' ), __( 'Global Add-ons', 'woocommerce-appointments' ), 'manage_woocommerce', 'global_addons', array( $this, 'global_addons_admin' ) );
	}
		...
		if ( ! empty( $_GET['add'] ) || ! empty( $_GET['edit'] ) ) {
			...
			if ( $_POST ) {
				...
				if ( $edit_id = $this->save_global_addons() ) {
					echo '<div class="updated"><p>' . __( 'Add-on saved successfully', 'woocommerce-appointments' ) . '</p></div>';
				}
			...
			if ( ! empty( $_GET['edit'] ) ) {
				...
				if ( ! $global_addon ) {
					echo '<div class="error">' . __( 'Error: Global Add-on not found', 'woocommerce-appointments' ) . '</div>';
					return;
				}
				...
				$reference           = __( 'Global Add-on Group', 'woocommerce-appointments' ) . ' #' . ( $global_addons_count->publish + 1 );
				$priority            = 10;
				$objects             = array( 0 );
			...
			if ( ! empty( $_GET['delete'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'delete_addon' ) ) {
				...
				echo '<div class="updated"><p>' . __( 'Add-on deleted successfully', 'woocommerce-appointments' ) . '</p></div>';
			}
	 ...
	 */
	public function tab() {
		?><li class="addons_tab product_addons"><a href="#product_addons_data"><span><?php _e( 'Add-ons', 'woocommerce-appointments' ); ?></span></a></li><?php
	}
