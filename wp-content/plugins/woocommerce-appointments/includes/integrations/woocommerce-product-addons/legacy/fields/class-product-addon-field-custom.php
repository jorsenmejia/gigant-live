		...
		foreach ( $this->addon['options'] as $key => $option ) {
			...
			switch ( $this->addon['type'] ) {
				...
				case "custom_letters_or_digits" :
					if ( ! empty( $option['min'] ) && ! empty( $posted ) && mb_strlen( $posted, 'UTF-8' ) < $option['min'] ) {
						return new WP_Error( 'error', sprintf( __( 'The minimum allowed length for "%s - %s" is %s.', 'woocommerce-appointments' ), $this->addon['name'], $option['label'], $option['min'] ) );
					}

					if ( ! empty( $option['max'] ) && ! empty( $posted ) && mb_strlen( $posted, 'UTF-8' ) > $option['max'] ) {
						return new WP_Error( 'error', sprintf( __( 'The maximum allowed length for "%s - %s" is %s.', 'woocommerce-appointments' ), $this->addon['name'], $option['label'], $option['max'] ) );
					}
				break;
				case "custom_price" :
				case "input_multiplier" :
					if ( ! empty( $option['min'] ) && ! empty( $posted ) && $posted < $option['min'] || ( isset( $option['min'] ) && $posted < $option['min'] ) ) {
						if ( ! empty( $this->addon['required'] ) ) {
							return new WP_Error( 'error', sprintf( __( 'The minimum allowed amount for "%s - %s" is %s.', 'woocommerce-appointments' ), $this->addon['name'], $option['label'], $option['min'] ) );
						}
					}

					if ( ! empty( $option['max'] ) && ! empty( $posted ) && $posted > $option['max'] ) {
						if ( ! empty( $this->addon['required'] ) ) {
							return new WP_Error( 'error', sprintf( __( 'The maximum allowed amount for "%s - %s" is %s.', 'woocommerce-appointments' ), $this->addon['name'], $option['label'], $option['max'] ) );
						}
					}
				break;
			}
			...
			switch ( $this->addon['type'] ) {
				case "input_multiplier" :
					...
					if ( $posted < 0 ) {
						return new WP_Error( 'error', sprintf( __( 'Please enter a value greater than 0 for "%s - %s".', 'woocommerce-appointments' ), $this->addon['name'], $option['label'] ) );
					}
				break;
				case "custom_letters_only" :
				...
				case "custom_email" :
					if ( ! empty( $posted ) && ! is_email( $posted ) ) {
						return new WP_Error( 'error', sprintf( __( 'A valid email address is required for "%s - %s".', 'woocommerce-appointments' ), $this->addon['name'], $option['label'] ) );
					}
				break;
			}
