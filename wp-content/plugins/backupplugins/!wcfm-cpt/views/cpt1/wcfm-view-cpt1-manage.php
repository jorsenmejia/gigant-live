...
if( isset( $wp->query_vars['wcfm-cpt1-manage'] ) && empty( $wp->query_vars['wcfm-cpt1-manage'] ) ) {
	if( !apply_filters( 'wcfm_is_allow_add_cpt1', true ) ) {
		wcfm_restriction_message_show( "Add " . WCFM_CPT_1_LABEL );
		return;
	}
	...
	if( $wcfm_cpt1_single->post_status == 'publish' ) {
		if( !apply_filters( 'wcfm_is_allow_edit_cpt1', true ) ) {
			wcfm_restriction_message_show( "Edit " . WCFM_CPT_1_LABEL );
			return;
		}
	}
	if( wcfm_is_vendor() ) {
		...
		if( !$is_cpt1_from_vendor ) {
			wcfm_restriction_message_show( "Restricted " . WCFM_CPT_1_LABEL );
			return;
		}
	}
}
...
<div class="collapse wcfm-collapse" id="">
  <div class="wcfm-page-headig">
		...
		<span class="wcfm-page-heading-text"><?php _e( 'Manage ' . WCFM_CPT_1_LABEL, 'wcfm-cpt' ); ?></span>
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			<h2><?php if( $cpt1_id ) { _e('Edit ' . WCFM_CPT_1_LABEL, 'wcfm-cpt' ); } else { _e('Add ' . WCFM_CPT_1_LABEL, 'wcfm-cpt' ); } ?></h2>
			<?php
			if( $cpt1_id ) {
				?>
			...
			if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
				?>
				<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('post-new.php?post_type='.WCFM_CPT_1); ?>" data-tip="<?php _e( 'WP Admin View', 'wcfm-cpt' ); ?>"><span class="fab fa-wordpress"></span></a>
				<?php
			}
				  ...
				  <div class="wcfm_cpt1_manager_general_fields">
						<?php
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_cpt1_manage_fields_general', array(
																																																"title" => array( 'placeholder' => __( WCFM_CPT_1_LABEL.' Title', 'wcfm-cpt') , 'type' => 'text', 'class' => 'wcfm-text wcfm_cpt1_title wcfm_full_ele', 'value' => $title),
																																													), $cpt1_id ) );
						...
						<?php if( !$wcfm_is_category_checklist = apply_filters( 'wcfm_is_category_checklist', true ) ) { ?>
						  <?php if( $wcfm_is_allow_category = apply_filters( 'wcfm_is_allow_category', true ) ) { $catlimit = apply_filters( 'wcfm_catlimit', -1 ); ?>
								...
								if( $wcfm_is_allow_custom_taxonomy = apply_filters( 'wcfm_is_allow_custom_taxonomy', true ) ) {
									$cpt1_taxonomies = get_object_taxonomies( WCFM_CPT_1, 'objects' );
