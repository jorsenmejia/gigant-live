...
//						'disable_subscribe_ip_antispam' => array('label' => __('Disable blocking Subscription from same IP', WPF_LANG_CODE), 'desc' => __('By default our plugin have feature to block subscriptions from same IP more then one time per hour - to avoid spam subscribers. But you can disable this feature here.', WPF_LANG_CODE), 'def' => '1', 'html' => 'checkboxHiddenVal'),
//						'disable_autosave' => array('label' => __('Disable autosave on PopUp Edit', WPF_LANG_CODE), 'desc' => __('By default our plugin will make autosave all your changes that you do in PopUp edit screen, but you can disable this feature here. Just don\'t forget to save your PopUp each time you make any changes in it.', WPF_LANG_CODE), 'def' => '0', 'html' => 'checkboxHiddenVal'),
//						'add_love_link' => array('label' => __('Enable promo link', WPF_LANG_CODE), 'desc' => __('We are trying to make our plugin better for you, and you can help us with this. Just check this option - and small promotion link will be added in the bottom of your PopUp. This is easy for you - but very helpful for us!', WPF_LANG_CODE), 'def' => '0', 'html' => 'checkboxHiddenVal'),
//						'access_roles' => array('label' => __('User role can use plugin', WPF_LANG_CODE), 'desc' => __('User with next roles will have access to whole plugin from admin area.', WPF_LANG_CODE), 'def' => 'administrator', 'html' => 'selectlist', 'options' => array($this, 'getAvailableUserRolesSelect'), 'pro' => ''),
//						'foot_assets' => array('label' => __('Load Assets in Footer', WPF_LANG_CODE), 'desc' => __('Force load all plugin CSS and JavaScript files in footer - to increase page load speed. Please make sure that you have correct footer.php file in your WordPress theme with wp_footer() function call in it.', WPF_LANG_CODE), 'def' => '0', 'html' => 'checkboxHiddenVal'),
//						'disable_email_html_type' => array('label' => __('Disable HTML Emails content type', WPF_LANG_CODE), 'desc' => __('Some servers fail send emails with HTML content type: content-type = "text/html", so if you have problems with sending emails from our plugn - try to disable this feature here.', WPF_LANG_CODE), 'def' => '0', 'html' => 'checkboxHiddenVal'),
//						'use_local_cdn' => array('label' => __('Disable CDN usage', WPF_LANG_CODE), 'desc' => esc_html(sprintf(__('By default our plugin is using CDN server to store there part of it\'s files - images, javascript and CSS libraries. This was designed in that way to reduce plugin size, make it lighter and easier for usage. But if you need to store all files - on your server - you can disable this option here, then upload plugin CDN files to your own site. To make it work correctly - check our article that describe how you need to do this <a href="%s" target="_blank">here</a>.', WPF_LANG_CODE), 'https://woobewoo.com/disable-cdn-usage-popup-plugin/')), 'def' => '0', 'html' => 'checkboxHiddenVal'),
//						'mail_send_engine' => array('label' => __('Send With', WPF_LANG_CODE), 'desc' => __('You can send your emails with different email sendng engines.', WPF_LANG_CODE), 'def' => 'wp_mail', 'html' => 'selectbox',
//							'options' => array('wp_mail' => __('WordPress PHP Mail', WPF_LANG_CODE), 'smtp' => __('Third party providers (SMTP)', WPF_LANG_CODE), 'sendmail' => __('Sendmail', WPF_LANG_CODE))),
//
//						'smtp_host' => array('label' => __('SMTP Hostname', WPF_LANG_CODE), 'desc' => __('e.g. smtp.mydomain.com', WPF_LANG_CODE), 'html' => 'text', 'connect' => 'mail_send_engine:smtp'),
//						'smtp_login' => array('label' => __('SMTP Login', WPF_LANG_CODE), 'desc' => __('Your email login', WPF_LANG_CODE), 'html' => 'text', 'connect' => 'mail_send_engine:smtp'),
//						'smtp_pass' => array('label' => __('SMTP Password', WPF_LANG_CODE), 'desc' => __('Your emaail password', WPF_LANG_CODE), 'html' => 'password', 'connect' => 'mail_send_engine:smtp'),
//						'smtp_port' => array('label' => __('SMTP Port', WPF_LANG_CODE), 'desc' => __('Port for your SMTP provider', WPF_LANG_CODE), 'html' => 'text', 'connect' => 'mail_send_engine:smtp'),
//						'smtp_secure' => array('label' => __('SMTP Secure', WPF_LANG_CODE), 'desc' => __('Use secure SMTP connection. If you enable this option - make sure that your server support such secure connections.', WPF_LANG_CODE), 'html' => 'selectbox', 'connect' => 'mail_send_engine:smtp',
//							'options' => array('' => __('No', WPF_LANG_CODE), 'ssl' => 'SSL', 'tls' => 'TLS'), 'def' => ''),
//
//						'sendmail_path' => array('label' => __('Sendmail Path', WPF_LANG_CODE), 'desc' => __('You can check it on your server, or ask about it - in your hosting provider.', WPF_LANG_CODE), 'html' => 'text', 'connect' => 'mail_send_engine:sendmail', 'def' => $defSendmailPath),
					),
				),
			));
