...
class WC_Appointments_Admin_Report_Dashboard {
	...
	public static function add_stats_to_dashboard() {
		...
		$require_confirmation = $wpdb->get_var(
			"SELECT COUNT(DISTINCT wcappointments.ID) AS count
				FROM {$wpdb->posts} AS wcappointments
		...
		<li class="low-in-stock">
			<a href="<?php echo esc_html( admin_url( 'edit.php?post_type=wc_appointment&post_status=pending-confirmation' ) ); ?> ">
				<?php
				/* translators: 0: number of appointments that require confirmation, 1: number of appointments that require confirmation */
				printf( wp_kses_post( _n( '<strong>%s appointment</strong> requires confirmation', '<strong>%s appointments</strong> require confirmation', $require_confirmation, 'woocommerce-appointments' ) ), esc_html( $require_confirmation ) );
				?>
			</a>
		</li>
		<?php
