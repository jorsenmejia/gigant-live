...
class Product_Addon_Groups {
	...
	static public function get_group( $id ) {
		...
		return new WP_Error( 'invalid_id', __( 'Unable to retrieve data. Invalid global add ons group or product ID.', 'woocommerce-appointments' ) );
	}
	...
	static public function delete_group( $id ) {
		if ( ! self::is_a_global_group_id( $id ) ) {
			return new WP_Error( 'invalid_id', __( 'Unable to delete group. Invalid global add ons group ID.', 'woocommerce-appointments' ) );
		}
