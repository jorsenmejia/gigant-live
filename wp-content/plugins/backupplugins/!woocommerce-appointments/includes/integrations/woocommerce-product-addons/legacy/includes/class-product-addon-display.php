...
class Product_Addon_Display {
	...
	public function styles() {
		...
		if ( is_singular( 'product' ) || class_exists( 'WC_Quick_View' ) ) {
			wp_enqueue_style( 'woocommerce-addons-css', WC_PRODUCT_ADDONS_PLUGIN_URL . '/assets/css/frontend.css' );
		}
	}
	...
	public function addon_scripts() {
		...
		$params = array(
			...
			'i18n_sub_total'               => __( 'Sub total:', 'woocommerce-appointments' ),
	...
	public function get_addon_html( $addon, $product_addons ) {
		...
		do_action( 'woocommerce-product-addons_get_' . $addon['type'] . '_html', $addon, $product_addons );
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_html( $addon, $product_addons ) {
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_letters_only_html( $addon, $product_addons ) {
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_digits_only_html( $addon, $product_addons ) {
		wc_get_template( 'addons/custom_pattern.php', array(
			'addons'  => $product_addons,
			'addon'   => $addon,
			'pattern' => '[0-9]*',
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_letters_or_digits_html( $addon, $product_addons ) {
		wc_get_template( 'addons/custom_pattern.php', array(
			'addons'  => $product_addons,
			'addon'   => $addon,
			'pattern' => '[A-Za-z0-9 ]*',
	 ...
	 * @param array $addon Add-on field data.
	 */
	public function get_custom_email_html( $addon, $product_addons ) {
