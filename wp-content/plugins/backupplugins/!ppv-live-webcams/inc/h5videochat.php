...
trait H5Videochat {
	...
	function videowhisper_cam_app($atts)
	{
		...
		if ($callsOnly && !$isPerformer && !$isCall) return  '<div class="ui segment red">' . __('This room can only be used for locked calls, with call link.', 'ppv-live-webcams') . '</div>';


		//create a session
		$session = self::sessionUpdate($userName, $roomName, $isPerformer, 11, 0, 1, 1, $options, $userID, $roomID, self::get_ip_address());
		...
		if (!$isCall)
		{
			...
			if ($atts['private'])
			{
				$pJS = ', requestUID: ' . get_post_meta( $postID, 'performerUserID', true ) . ", requestUsername: '" . get_post_meta( $postID, 'performer', true ) . "'";
				$isCallInstant = 1;
			}
		}

		$group_disabled = self::is_true(get_post_meta($postID, 'group_disabled', true ));
		if ($callsOnly && !$isPerformer && !$isCall && !$isCallInstant) return  '<div class="ui segment red">' . __('Group chat is disabled. This room can only be used for instant or locked calls.', 'ppv-live-webcams') . '</div>';
			...
			if ( strpos($filename,'.js') && !strpos($filename,'.js.map')) // && !strstr($filename,'runtime~')
				{
					...
					if (substr($filename,0,5) == 'main.') $countMain++;
					if (substr($filename,0,7) =='runtime') $countRuntime++;
				}
...
HTMLCODE;
	...
	static function appRoomOptions($post, $session, $options)
	{
		...
		if ($session->broadcaster) //room owner
			...
			'calls_only' => [ 'name'=>'calls_only', 'description' => __('Calls Only', 'ppv-live-webcams'), 'details'=>__('Call Only mode: can only be used for calls.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => self::is_true(get_post_meta($post->ID, 'calls_only', true )) ],
			'group_disabled' => [ 'name'=>'group_disabled', 'description' => __('Group Chat Disabled', 'ppv-live-webcams'), 'details'=>__('Use room only for instant or locked calls.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => self::is_true(get_post_meta($post->ID, 'group_disabled', true )) ],

			'room_audio' => [ 'name'=>'room_audio', 'description' => __('Audio Only', 'ppv-live-webcams'), 'details'=>__('Audio only room mode: Only microphone, no webcam video, for all participants. Applies both to group and private calls.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => $audio = self::is_true(get_post_meta($post->ID, 'room_audio', true )) ],
			...
			'party' => [ 'name'=>'party', 'description' => __('Party Mode', 'ppv-live-webcams'), 'details'=>__('In party mode, all users that join room are added to party squad. Entire party moves to different room with Next button.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' =>  $party = self::is_true(get_post_meta($post->ID, 'party', true )) ],
			...
			if ($collaboration || $conference) $fields['room_slots'] = [ 'name'=>'room_slots', 'description' => __('Display Slots', 'ppv-live-webcams'), 'details'=>__('Split display to show multiple media items, in conference mode.', 'ppv-live-webcams'), 'type'=> 'dropdown', 'value' => $room_slots, 'options' => [
				...
				$fields['party_reserved'] = [ 'name'=>'party_reserved', 'description' => __('Party Reserved', 'ppv-live-webcams'), 'details'=>__('Number of reserved party video slots, needs to be available in other room, to travel.', 'ppv-live-webcams'), 'type'=> 'dropdown', 'value' => $party_reserved, 'options' => [
				['value'=>'0', 'text'=>'None'],
				['value'=>'1', 'text'=>'1'],
				['value'=>'2', 'text'=>'2'],
				['value'=>'4', 'text'=>'4'],
			...
			$configuration['room'] =[ 'name' =>  __('Room Preferences', 'ppv-live-webcams') . ': '. $post->post_title, 'fields' => $fields];
		}

		//user options
		$fieldsUser = [
		'h5v_audio' => [ 'name'=>'h5v_audio', 'description' => __('Audio Only', 'ppv-live-webcams'), 'details'=>__('Audio only user mode: Publish only microphone, no webcam video. Applies both to group and private calls.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => $userAudio = self::is_true(get_user_meta($session->uid, 'h5v_audio', true )) ],
		'h5v_dark' => [ 'name'=>'h5v_dark', 'description' => __('Dark Mode', 'ppv-live-webcams'), 'details'=>__('Dark interface mode.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => $darkMode = self::is_true(get_user_meta($session->uid, 'h5v_dark', true )) ],
		'h5v_reveal' => [ 'name'=>'h5v_reveal', 'description' => __('Reveal Mode', 'ppv-live-webcams'), 'details'=>__('Webcam reveal mode: hide webcam in private calls and use a Reveal button to enable on request.', 'ppv-live-webcams'), 'type'=> 'toggle', 'value' => $revealMode = self::is_true(get_user_meta($session->uid, 'h5v_reveal', true )) ],
		];
		...
		if ($revealMode)
		{
			...
			$fieldsUser['h5v_reveal_warmup'] = [ 'name'=>'h5v_reveal_warmup', 'description' => __('Reveal Warmup', 'ppv-live-webcams'), 'details'=>__('Time in seconds required before revealing webcam is possible.', 'ppv-live-webcams'), 'type'=> 'dropdown', 'value' => $h5v_reveal_warmup, 'options' => [
		...
		$configuration['user'] =[ 'name' =>  __('User Preferences', 'ppv-live-webcams') . ': '. $session->username, 'fields' => $fieldsUser];
	...
	static function appPublicRoom($post, $session, $options, $welcome ='', &$room = null, $requestUID = 0)
	{
		...
		elseif ($session->broadcaster) //member: performer
			{
			$room['welcome'] .= "\n".__('You are room performer. Registered users can request private calls and you can accept:', 'ppv-live-webcams');
			...
			if ($clientCPM && $performerRatio) $room['welcome'] .= "\n - " . __('Private call earning per minute, for performer:', 'ppv-live-webcams') . ' ' . number_format($clientCPM*$performerRatio, 2, '.','') . ' ' . htmlspecialchars($options['currencypm']);
		}
		else //member: client
			{
		...
		if ($party) $room['welcome'] .= "\n " .  __('This room is in party mode and all users join the party squad. Entire party can be moved to other rooms by host.', 'ppv-live-webcams');

	
			if (!is_array($userMeta = unserialize($session->meta))) $userMeta = array();
			if (array_key_exists('party', $userMeta)) if ($userMeta['party'])
			{
				...
				else $room['next'] = false; //only host can move party
		...
		if ($conference || $collaboration || $appComplexity)
		{
			...
			if ($options['appOptions']) if ($session->broadcaster)
				{
		...
		$room['panelBroadcast'] = $room['panelCamera'] && $session->broadcaster;
		
		if ($room['panelBroadcast']) $room['broadcastSettings'] = self::appBroadcastSettings($session, $post, $options);
	...
	static function appBroadcastSettings($session, $post, $options)
	{
		$configuration = [];
			...
			$fields = [
			'streamURL' => [ 'name'=>'streamURL', 'description' => __('Stream URL', 'ppv-live-webcams'), 'details'=>__('RTMP Address / OBS Stream URL: full streaming address. Contains: server, port if different than default 1935, application and control parameters, key.', 'ppv-live-webcams'), 'type'=> 'text', 'value' =>  $rtmpAddress ],
			'streamKey' => [ 'name'=>'streamKey', 'description' => __('Stream Key', 'ppv-live-webcams'), 'details'=>__('Stream Name / OBS Stream Key: name of stream.'), 'type'=> 'text', 'value' => $stream ],
			];
			
		$configuration['rtmp_obs'] =[ 'name' =>  __('RTMP Encoder / OBS Settings', 'ppv-live-webcams'), 'details' => __('Use these settings to broadcast with a RTMP encoder app like OBS.', 'ppv-live-webcams'), 'fields' => $fields];
	...
	function vmls_app()
	{
		...
		{
			//check session
			if (! $session = self::sessionValid($sessionID, $userID)) self::appFail('Session invalid or closed. Usually occurs if browser tab gets paused in background or room type changes. Please RELOAD! Session #' . $sessionID . ' User #' .  $userID . ' Room #' . $roomID .' ' . $task );
		...
		case 'options':
			...
			if (!in_array($name, ['requests_disable', 'room_private', 'calls_only', 'group_disabled', 'room_slots', 'room_conference', 'room_audio', 'vw_presentationMode', 'h5v_audio', 'h5v_dark', 'h5v_reveal', 'h5v_reveal_warmup', 'party', 'party_reserved'])) self::appFail('Preference not supported!');
		...
		case 'party_return':
			...
			if (array_key_exists('party', $userMeta)) //is in a party
			{
				...
				$nextSession = self::sessionUpdate($userName, $nextPost->post_title, 0, 11, 0, 1, 1, $options, $userID, $nextRoomID, self::get_ip_address());
				$nextSessionID = $nextSession->id;
					
				$userMetaS = serialize($userMeta);
				$sql = "UPDATE `$table_sessions` set meta='$userMetaS' WHERE id ='" . $nextSession->id . "'";
				$wpdb->query($sql);
		...
		if (array_key_exists('party', $userMeta)) //is in a party
			...
			if (!$partyRoomID) $partyRoomID = $partyID; //party mode
			
			if ($postID != $partyRoomID) //move to new party room
				{
				...
				$nextSession = self::sessionUpdate($userName, $nextPost->post_title, 0, 11, 0, 1, 1, $options, $userID, $partyRoomID, self::get_ip_address());
				$nextSessionID = $nextSession->id;
			...
			if ($party) //in party start room: add session to party
				{
		...
		if ($task != 'login')
		{
			...
			if (!array_key_exists('user', $response))
				$response['user'] =
					[
				'loggedIn'=> true,
				'balance' => number_format( $balance = self::balance($userID, true, $options),2),
				];
				...
				{
					$warnTime = 0;		
					$lastWarning = intval(get_user_meta($userID, 'balance_warning', true));
					if (time() - $lastWarning>600) $warnTime = 1; //warn coolddown 10 min
					...
					if ($balance > 0 && $warnTime && ($balance < $options['balanceWarn1Amount'] || $balance < $options['balanceWarn2Amount']) )
					{
						...
						if ($notify)
						{	
							self::notificationMessage($notifyMessage, $session, $privateUID, ['type'=> $notify, 'sound' => $notifySound]);
							update_user_meta($userID, 'balance_warning', time());
						}
					}
				}
			...
			if ($updateTime) if ($updateTime > $lastRoomUpdate) $needUpdate['user'] = 1;
				if ($needUpdate['user']) $response['user']['options'] = self::appUserOptions($session, $options);
