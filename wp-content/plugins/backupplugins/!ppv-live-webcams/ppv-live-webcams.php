...
*/
		...
		function the_content($content)   //! webcam post page
			...
			if ( in_array($view, array('profile', 'content', 'messages')) )
			{
				...
				if ($view == 'messages')  $addCode .= self::webcamMessages($postID, $options, $room);
			...
			$isPerformer = VWliveWebcams::isAuthor($postID);
		...
		static function webcamLinks($postID, $options)
		{
			...
			$addCode .= '<a class="ui button tiny compact" href="'. add_query_arg('view', 'content',get_permalink( $postID)) .'">' .'Webcam Profile Content'. '</a> ' ;
			$addCode .= '<a class="ui button tiny compact" href="'. add_query_arg('view', 'profile',get_permalink( $postID)) .'">' .'Webcam Profile Info'. '</a> ' ;
			
			if ($options['messages']) $addCode .= '<a class="ui button tiny compact" href="'. add_query_arg('view', 'messages',get_permalink( $postID)) .'">' .'Messages'. '</a> ' ;
    ...
    <i class="asterisk icon"></i>
			...
			if (count($postslist)>0)
			{
				...
				foreach ( $postslist as $item )
				{
					...
					{
						if (!$group_disabled) $enterCode .= '<a href="'.$url.'">' .$groupMode. ($groupCPM?' ' .$groupCPM . $options['currencypm']:''). '</a>';
				

						//special modes
						$enterCode .= '<a href="'  . $urlCall . '">' .__('Call', 'ppv-live-webcams') . ' ' . ($clientCPM ? $clientCPM . $options['currencypm'] : '').'</a>';
						
						if ($options['messages'])
						{
						$messagesCost = $options['messagesCost'];
						$enterCode .= '<a href="'  .add_query_arg('view', 'messages', $url) . '">' .__('Question', 'ppv-live-webcams') . ' ' . ($messagesCost ? $messagesCost . $options['currency'] : '').'</a>';
						}
						...
						if (is_array($options['groupModes']) && $mode = sanitize_text_field($_POST['groupMode']))
						{
							foreach ($options['groupModes'] as $groupMode => $modeParameters)
								if ($mode == $groupMode)
								{
									...
									foreach (['room_private', 'requests_disable ', 'room_audio', 'room_conference', 'room_slots', 'vw_presentationMode', 'calls_only', 'group_disabled'] as $meta)
									if (array_key_exists($meta, $modeParameters))
									{
										update_post_meta($postID, $meta, $modeParameters[$meta]);
										$htmlCode .= '<BR> - ' . $meta . ': ' . $modeParameters[$meta];
									}
...
//go-live room option defaults
			...
			if ($options['webrtc'])
			{
				...
				//html5 videochat app
				$interfaceCode .= '<OPTION value="html5app" ' . ($roomInterface=='html5app'?'selected':'') . '>' . __('HTML5', 'ppv-live-webcams') . '</OPTION>';
				$interfaceInfo .= ' * ' . __('HTML5 Videochat works in mobile and PC browsers, providing streaming with group chat, private 2 way videochat, tips.', 'ppv-live-webcams');
			...
			if ($options['webrtc'] < 5)
				if (!$iOS && !$Android)
				{
					...
					$interfaceInfo .= '<br> * ' . __('Advanced PC interface is Flash based and supports presentation mode, streaming with group chat, 2 way videochat, tips. Mobile users will get the HTML5 Streaming interface if transcoding is available.', 'ppv-live-webcams');
				}
...
{
			$checked = '';
			if ($_GET['messages']) $checked = 'active';
			if ($checked) $checked1 = true;
			
			global $wpdb;
			$table_messages = $wpdb->prefix . "vw_vmls_messages";
			$sqlC = "SELECT COUNT(id) FROM `$table_messages` WHERE webcam_id='$postID' AND reply_id='0' AND ldate='0' "; //for webcam, only questions (no replies), not replied (ldate)
		...
		function vmls_callback()
		{
			...
			//remove_filter('template_redirect','redirect_canonical');
				...
				if (is_array($rtpsessions))
					...
					if ($options['debugMode']) $debugInfo .= "\r\n - RTP Session: ($username #$userID @ $r #$postID) " . serialize($session);
				...
				if (is_array($users))
					foreach ($users as $user)
					{
							...
							if ($options['ffmpegTimeout'])
								if ($user['runSeconds']) if ($user['runSeconds']>$options['ffmpegTimeout'])
									...
									if ($user['role'] == '1') //channel broadcaster
										...
										$session = self::sessionUpdate($username, $r, 1, 9, 0, 0, 0, $options); //not strict in case this is existing flash user
										...
										if ($session->type >= 2) //external broadcaster: update here, otherwise updates on calls from flash app
											...
											if ($options['bannedNames'])
												if ($ban =  self::containsAny($r,$options['bannedNames'])) $disconnect = "Room name banned ($ban)!";


											//generate external snapshot for external broadcaster
											if ($session->broadcaster) self::streamSnapshot($session->session, true, $postID);
											...
											if ($postID && !$transcoding)
											{
												...
												update_post_meta($postID, 'stream-type', 'external');
												update_post_meta($postID, 'stream-updated', $ztime);
												//update_post_meta($postID, 'stream-external', $ztime);

												//type changed
												if ($streamType != 'external') update_post_meta($postID, 'updated_stream', $ztime);	
									...
									$session = self::sessionUpdate($username, $r, 0, 10, 0, 0, 0, $options); //not strict in case this is existing flash user
						...
						if ($options['debugMode']) $debugInfo .= "\r\n - RTMP Session: ($username @ $r #$postID) " . serialize($session);
...
*/
	...
	*/
		...
		function admin_bar_menu($wp_admin_bar)
		{
			...
			if ($options['p_videowhisper_webcams_client'])
				if (VWliveWebcams::any_in_array( array( $options['roleClient'], 'administrator', 'super admin'), $current_user->roles))
					$wp_admin_bar->add_node(array(
							'parent' => 'my-account-with-avatar',
...
<?php
			...
			if ($filterMode == 'private')
			{
				...
				if ($wpdb->num_rows>0)
				{
					...
					{
						echo '<tr>';
						echo '<td>' . $session->session .  '<BR>'. $session->ip .  '<BR>'. ($session->broadcaster?'Broadcaster':'Client') .' #'.$session->uid.  '</td>';
						echo '<td>' . $session->room . '<BR>'. $session->rid .'</td>';
						...
						echo '<td>' . print_r(unserialize($session->meta), true) . '</td>';
