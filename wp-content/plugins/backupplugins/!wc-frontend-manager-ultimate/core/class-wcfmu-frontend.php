...
class WCFMu_Frontend {
	...
	function wcfm_order_customer_add_note( $order_id ) {
		...
		if ( isset( $_POST['wcfm_cus_note_submit'] ) ) {
			...
			if ( $_FILES ) {
				...
				if ( $movefile && ! isset( $movefile['error'] ) ) {
					if ( !empty( $movefile['url'] ) ) {
						$final_note .= '<p><a target="_blank" class="wcfm_dashboard_item_title" href ="' . $movefile['url'] . '">' . __( 'See Attachment', 'wc-frontend-manager-ultimate' ) . '</a></p>';
					}
				} 
			}
			...
			if( $final_note ) {
				...
				_e( 'Your note succesfully submited.', 'wc-frontend-manager-ultimate' );
	...
	function wcfmu_product_manage_fields_variations( $variation_fileds, $variations, $variation_shipping_option_array, $variation_tax_classes_options ) {
		...
		$wcfmu_variation_fields = array(
																		...
																		"downloadable_file" => array('label' => __('File', 'wc-frontend-manager-ultimate'), 'type' => 'upload', 'mime' => 'doc', 'button_class' => 'downloadable_product', 'class' => 'wcfm-text wcfm_ele wcfm_half_ele variable variable-subscription variation_downloadable_ele', 'label_class' => 'wcfm_title wcfm_half_ele_title variation_downloadable_ele'),
	...
	function wcfmu_category_attribute_mapping_settings( $wcfm_options ) {
		global $WCFM, $WCFMu;
		$wcfm_category_attributes_mapping = get_option( 'wcfm_category_attributes_mapping', array() );
		$attribute_taxonomies = wc_get_attribute_taxonomies();
		?>
		<!-- collapsible -->
	...
	function wcfmu_category_attribute_mapping_settings_update( $wcfm_settings_form ) {
		...
		if( isset( $wcfm_settings_form['wcfm_category_attributes_mapping'] ) ) {
			...
			update_option( 'wcfm_category_attributes_mapping',  $wcfm_category_attributes_mapping );
		}
	}
