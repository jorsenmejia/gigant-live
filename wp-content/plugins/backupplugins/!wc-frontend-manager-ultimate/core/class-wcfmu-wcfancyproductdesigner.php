  ...
  function wcfpd_wcfm_menus( $menus ) {
  	global $WCFM;
		...
		$menus = array_slice($menus, 0, 3, true) +
												array( 'wcfm-fncy-product-designer' => array(   'label'  => __( 'Product Designer', 'woocommerce-appointments'),
  ...
  public function wcfpd_load_scripts( $end_point ) {
	  ...
	  switch( $end_point ) {
	  	case 'wcfm-products-manage':
			...
			case 'wcfm-orders-details':
				...
				wp_enqueue_script( 'wcfm-fpd-order-viewer', $WCFMu->library->js_lib_url . 'wc_fncy_product_designer/wcfmu-script-fancy-order-viewer.js', array( 'jquery-fpd' ), Fancy_Product_Designer::VERSION);
      	...
      	wp_enqueue_media();
				...
				wp_localize_script( 'wcfm-fpd-manage-fancy-products', 'fpd_admin_opts', array(
						'adminAjaxUrl' => admin_url('admin-ajax.php'),
						'ajaxNonce' => wp_create_nonce( 'fpd_ajax_nonce' ),
						'enterTitlePrompt' => __('Please enter a title!', 'radykal'),
						'tryAgain' => __('Something went wrong. Please try again!', 'radykal'),
      ...
      case 'wcfm-fncy-product-builder':
      	...
      	$WCFM->library->load_colorpicker_lib();
				...
				wp_localize_script( 'wcfm-fpd-product-builder', 'fpd_admin_opts', array(
						'adminAjaxUrl' => admin_url('admin-ajax.php'),
						'ajaxNonce' => wp_create_nonce( 'fpd_ajax_nonce' ),
						'enterTitlePrompt' => __('Please enter a title!', 'radykal'),
						'tryAgain' => __('Something went wrong. Please try again!', 'radykal'),
	...
	public function wcfpd_load_styles( $end_point ) {
	  ...
	  switch( $end_point ) {
	  	case 'wcfm-products-manage':
			...
			case 'wcfm-fncy-product-designer':
	    	wp_enqueue_style( 'wcfmu_appointments_dashboard_css',  $WCFMu->library->css_lib_url . 'wc_fncy_product_designer/wcfmu-style-manage-fancy-products-designer.css', array(), $WCFMu->version );
		  break;
		  
		  case 'wcfm-fncy-product-builder':
	    	wp_enqueue_style( 'wcfmu_appointments_css',  $WCFMu->library->css_lib_url . 'wc_fncy_product_designer/wcfmu-style-manage-fancy-products-builder.css', array(), $WCFMu->version );
		  break;
	  }
	}
