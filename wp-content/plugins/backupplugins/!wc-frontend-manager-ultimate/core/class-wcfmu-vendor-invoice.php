  		...
  		foreach( $wcfm_store_invoices as $vendor_id => $wcfm_store_invoice ) {
				if( file_exists( $wcfm_store_invoice ) ) {
					if (empty($upload_dir['error'])) {
						...
						$actions['wcfm-store-invoice-'.$vendor_id] = array( 'name' => apply_filters( 'wcfm_store_invoice_download_label', $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_name_by_vendor( absint($vendor_id) ) . ' ' . $sold_by_text . ' ' . __( 'Invoice', 'wc-frontend-manager-ultimate' ), $order->get_id(), $vendor_id ), 'url' => $invoice_path );
					}
				}
			}
		}
