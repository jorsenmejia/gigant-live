		...
		if(!empty($wcfm_followerss_array)) {
			...
			foreach($wcfm_followerss_array as $wcfm_followerss_single) {
				...
				if( apply_filters( 'wcfmmp_is_allow_sold_by_linked', true ) ) { 
					$wcfm_followerss_json_arr[$index][] = $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( $wcfm_followerss_single->user_id ); 
				} else { 
					$wcfm_followerss_json_arr[$index][] = $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_name_by_vendor( absint($wcfm_followerss_single->user_id) ); 
				}
