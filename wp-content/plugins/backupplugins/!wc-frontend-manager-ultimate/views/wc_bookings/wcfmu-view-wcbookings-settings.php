...
if( isset( $wp->query_vars['wcfm-bookings-settings'] ) ) {
	...
	$availability_rules = get_option( 'wc_global_booking_availability', array() );
	
	$availability_table = $wpdb->query( "SHOW tables like '{$wpdb->prefix}wc_bookings_availability'");
	if( $availability_table ) {
		$availability_rules = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}wc_bookings_availability" );
		...
		if( !empty( $availability_rules ) ) {
			foreach( $availability_rules as $a_index => $availability_rule ) {
				if( !wcfm_is_vendor() || ( wcfm_is_vendor() && ( $WCFM->is_marketplace == 'wcpvendors' ) && ! empty( $availability_rule['vendor'] ) && ( (int) WC_Product_Vendors_Utils::get_logged_in_vendor() === $availability_rule['vendor'] ) ) ) {
					...
					$availability_rule_values[$a_index]['type'] = $availability_rule->range_type;
					if($availability_rule->range_type == 'custom' ) {
						$availability_rule_values[$a_index]['from_custom'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_custom']   = $availability_rule->to_range;
					} elseif($availability_rule->range_type == 'months' ) {
						$availability_rule_values[$a_index]['from_months'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_months']   = $availability_rule->to_range;
					} elseif($availability_rule->range_type == 'weeks' ) {
						$availability_rule_values[$a_index]['from_weeks'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_weeks']   = $availability_rule->to_range;
					} elseif($availability_rule->range_type == 'days' ) {
						$availability_rule_values[$a_index]['from_days'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_days']   = $availability_rule->to_range;
					} elseif($availability_rule->range_type == 'time:range' ) {
						...
						$availability_rule_values[$a_index]['from_time'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_time']   = $availability_rule->to_range;
					} else {
						$availability_rule_values[$a_index]['from_time'] = $availability_rule->from_range;
						$availability_rule_values[$a_index]['to_time']   = $availability_rule->to_range;
					}
					$availability_rule_values[$a_index]['av_title'] = $availability_rule->title;
