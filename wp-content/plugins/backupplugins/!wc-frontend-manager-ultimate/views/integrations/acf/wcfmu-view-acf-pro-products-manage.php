				...
				<div id="wcfm_products_manage_form_<?php echo sanitize_title($field_group['title']); ?>_expander" class="wcfm-content">
				  ...
				  if ( !empty( $field_group_fields ) ) {
				  	foreach( $field_group_fields as $field_group_field ) {
				  		if( !isset( $field_group_field['name'] ) || empty( $field_group_field['name'] ) ) continue;
				  		$field_value = '';
				  		...
				  		switch( $field_group_field['type'] ) {
							  ...
							  break;
								...
								case 'message':
									$WCFM->wcfm_fields->wcfm_generate_form_field(  array( $field_group_field['name'] => array( 'label' => $field_group_field['label'], 'type' => 'html', 'class' => 'wcfm-html wcfm_ele simple variable external grouped booking ' . $field_group_field['key'], 'label_class' => 'wcfm_title ' . $field_group_field['key'], 'value' => wpautop( $field_group_field['message'] ) ) ) );
								break;
