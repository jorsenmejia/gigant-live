...
<?php if( $document->get_header_logo_id() ) echo apply_filters( 'wcfm_withdrawal_invoice_invoice_title', __( 'Withdrawal Invoice', 'wc-frontend-manager-ultimate' ) ); ?>
</h1>
		...
		<td class="order-data">
			<table>
				<tr class="order-number">
					...
					<td>#<?php echo sprintf( '%06u', $transaction_id ); ?></td>
				</tr>
				<tr class="order-date">
	  	...
	  	foreach( $wi_commission_ids as $wi_commission_id ) {
				...
				if( !empty( $orders_details ) ) {
					...
					<tr>
						...
						<?php } ?>
						<td style="text-align:center;"><?php echo wc_price( ( $commission_tax + $order_details->total_commission ), array( 'currency' => $order_currency ) ); ?></td>
					</tr>
					<?php
					}
				}
					  ...
					  <tr class="no-borders">
						...
						<?php if( isset( $commission_rule['tax_enable'] ) && ( $commission_rule['tax_enable'] == 'yes' ) ) { ?>
							<tr class="no-borders">
								...
								<td style="text-align:right;float:right;padding-top:10px;"><?php echo wc_price( ( $total_commission_tax + $transaction->withdraw_amount ), array( 'currency' => $pay_currency ) ); ?></td>
							</tr>
							<tr class="no-borders">
								<td style="text-align:right;float:right;"><?php echo $commission_rule['tax_name'] . ' ('. $commission_rule['tax_percent'] .'%)'; ?></td>
								<td style="text-align:right;float:right;"><?php echo '-' . wc_price( $total_commission_tax, array( 'currency' => $pay_currency ) ); ?></td>
							</tr>
						<?php } ?>
						<tr class="no-borders">
