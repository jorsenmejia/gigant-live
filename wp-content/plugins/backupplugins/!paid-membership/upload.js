// VideoShareVOD.com Multi Drag & Drop File Upload Script
...
(function() {
	...
	function vwUF(file) {
		...
		if (xhr.upload && file.size > 0 && file.size <= vwEBI("MAX_FILE_SIZE").value) 		{
			var o = vwEBI("progress");
			var progress = o.appendChild(document.createElement("div"));
			progress.appendChild(document.createTextNode(encodeURIComponent(file.name) + ": " ));
			var msg = "MIME Type: " + file.type + " Size: " + file.size + " bytes : Uploading. Please wait...";
			...
			xhr.setRequestHeader("X-COLLECTION", vwEBI('collection').value.replace(/(<([^>]+)>)/ig,""));
			xhr.setRequestHeader("X-PICTURE", vwEBI('picture').value.replace(/(<([^>]+)>)/ig,""));
