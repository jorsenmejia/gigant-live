	...
	protected function get_review_data() {
		...
		$reviews = array_map( function( $review ) {
			...
			if( 'yes' === get_option( 'ivole_google_encode_special_chars', 'no' ) ) {
				$_review->author = mb_convert_encoding( htmlspecialchars( $_review->author, ENT_XML1, 'UTF-8' ), 'HTML-ENTITIES' );
				//$_review->author = mb_convert_encoding( esc_html( $_review->author ), 'HTML-ENTITIES' );
			} else {
				$_review->author = htmlspecialchars( $_review->author, ENT_XML1, 'UTF-8' );
				//$_review->author = esc_html( $_review->author );
			}
