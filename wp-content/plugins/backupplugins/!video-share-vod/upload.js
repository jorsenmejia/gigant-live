...
(function() {
	...
	function vwUF(file) {
		...
		progress.appendChild(document.createTextNode(msg));
			...
			xhr.setRequestHeader("X-CATEGORY", encodeURIComponent(vwEBI('category').value));
			xhr.setRequestHeader("X-OWNER", encodeURIComponent(vwEBI('owner').value));
			xhr.setRequestHeader("X-DEV", 'Uploaded with VideoShareVOD.com script by VideoWhisper.com');
			xhr.setRequestHeader("X-PLAYLIST", encodeURIComponent(vwEBI('playlist').value));
			xhr.setRequestHeader("X-TAG", encodeURIComponent(vwEBI('tag').value));
			xhr.setRequestHeader("X-DESCRIPTION", encodeURIComponent(vwEBI('description').value));
			xhr.send(file);
		}
