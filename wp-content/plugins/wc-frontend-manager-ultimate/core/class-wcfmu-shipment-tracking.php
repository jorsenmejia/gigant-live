      ...
      if( $tracking_code && $tracking_url ) {
				if( wcfm_is_vendor() ) {
					...
					foreach( $product_ids as $product_id ) {
						...
						$comment_id = $order->add_order_note( $wcfm_messages, apply_filters( 'wcfm_is_allow_shipment_note_to_customer', '1' ) );
						add_comment_meta( $comment_id, '_vendor_id', $user_id );
	...
	function wcfm_mark_as_recived() {
      ...
      // Keep Tracking URL as Order Item Meta
			...
			if( apply_filters( 'wcfm_is_allow_order_complete_on_receive', false ) ) {
				...
				$wcfm_messages = sprintf( __( '<b>%s</b> order status updated to <b>%s</b> on item receive by customer.', 'wc-frontend-manager-ultimate' ), '#<a target="_blank" class="wcfm_dashboard_item_title" href="' . get_wcfm_view_order_url($order_id) . '">' . $order_id . '</a>', wc_get_order_status_name( 'completed' ) );
