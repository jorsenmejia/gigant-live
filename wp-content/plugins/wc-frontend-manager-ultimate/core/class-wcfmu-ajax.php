		...
		if ( $order_id > 0 ) {
			...
			if( apply_filters( 'wcfm_is_allow_order_note_attachments', true ) ) {
				...
				if( !empty( $attachments ) ) {
					$note .= "<br />" . __	( 'Attachments', 'wc-frontend-manager-ultimate' ) . ': ';
					foreach( $attachments as $index => $attachment ) {
						if( isset( $attachment['file'] ) && !empty( $attachment['file'] ) ) {
							...
							if( $index != 0  ) $note .= ',&nbsp;';
							$note .= '<a class="wcfm_dashboard_item_title wcfm_linked_attached" target="_blank" href="' . $attachment['file'] . '">' . $name . '</a>';
						}
					}
				}
			}
