...
(function($) {
	...
	$('#fooevents_seating_options_table').on('click', '.fooevents_seating_remove', function(event) {
		fooevents_seating_delete_row_field($(this));
		return false;
	});

	$('#fooevents_seating_options_table').on('keyup', '.fooevents_seating_row_name, .fooevents_seating_options', function(event) {
		clearTimeout(typing_timer);
		typing_timer = setTimeout(fooevents_update_row_row_ids, done_typing_interval, $(this));
		return false;
	});

	
	$('#fooevents_seating_options_table').on('keydown', '.fooevents_seating_row_name, .fooevents_seating_options', function(event) {
		clearTimeout(typing_timer);
	});

	$('#fooevents_seating_options_table').on('change', '.fooevents_seating_variations', function(event) {
		fooevents_serialize_options_seating();
	});
	
	$('#fooevents_seating_options_table').on('change', '.fooevents_seating_number_seats', function(event) {
		fooevents_serialize_options_seating();
	});
...
jQuery('#fooevents_seating_chart').on('click', function(){
	...
	$("#fooevents_seating_options_table tbody tr").each(function(){
			rowName = $(this).find(".fooevents_seating_row_name").val();
			rowID = $(this).find(".fooevents_seating_row_name").attr("id");
			numberSeats = $(this).find(".fooevents_seating_number_seats").val();
			$("#fooevents_seating_dialog").append("<div class='fooevents_seating_chart_view_row_name' id='" + rowID + "'>" + rowName + "</div>");
			seats = $('<div>', { 'class': 'fooevents_seating_chart_view_row' });
			
			unavailableSeatsID = $('input[value="fooevents_seats_unavailable_serialized"]').attr("id");
			if (unavailableSeatsID !== undefined) {
					unavailableSeatsID = unavailableSeatsID.substr(0, unavailableSeatsID.lastIndexOf("-")) + "-value";
					unavailableSeats = $("#" + unavailableSeatsID).html();
			} else {
					unavailableSeatsID = "";
			}
			...
			for(var i = 1; i <= numberSeats; i++) {
					...
					$(seats).append("<span class='" + seatClass + "'>" + i + "</span>");
					seatClass = "available";
			}

			
			$("#fooevents_seating_dialog").append(seats);
		
			
	});
	if( $("#fooevents_seating_dialog").is(':empty') ) {
			$("#fooevents_seating_dialog").append("<div style='margin-top:20px'>No seats to show. Add rows and seats by clicking on the '+ New Row' button.</div>");
	}
	
	$("#fooevents_seating_dialog").dialog({
			width: "50%",
			maxWidth: "768px",
			height: "auto",
			maxHeight: "768px",
...
function get_variations(opt_num) {
	...
	the_variations = jQuery.post(ajaxurl, dataVariations, function(response) {
	  ...
	  $("#" + opt_num + "_WooCommerceEventsSelectedVariation").append(data);
	});
}
