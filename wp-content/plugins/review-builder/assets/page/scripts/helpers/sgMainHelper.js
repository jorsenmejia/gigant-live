...
SGMainHelper.prototype.init = function () {
	...
	if (that.getURLParameter('edit')) {
		if (jQuery('.sgrb-text-input.sgrb-title-input.sgrb-reviewupdate-js').length) {
...
SGMainHelper.prototype.showSuccessBanner = function (text) {
	if (text == 'Review updated') {
		var successBannerWrapper = jQuery('.sgrb-js-form').parent();
		jQuery('<div class="alert alert-success is-dismissible notice" role="alert">' +
		'<h4>'+text+'</h4>' +
		'<button type="button" class="notice-dismiss" onclick="jQuery(\'.notice\').remove();"></button></div>').prependTo(successBannerWrapper);
	}
	else {
