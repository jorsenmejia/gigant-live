...
trait Requirements {
	...
	static function requirementsDefinitions()
	{
		...
		$broadcastPageID= $options['p_videowhisper_webcams_performer'];
		...
		return array(
			...
			(
				...
				'fix' => 'Go to Performer Dashboard and Go Live using Advanced (PC) interface to test PC Flash based RTMP broadcasting.',
				'url' => $broadcastPage,
			),
			...
			(
				...
				'fix' => 'Deploy solution on <a href="https://webrtchost.com/hosting-plans/#Complete-Hosting" target="_blank">Complete Turnkey Streaming Hosting from WebRTChost.com</A> (recommended) for full capabilities or add only WebRTC relay streaming service using <A href="https://webrtchost.com/hosting-plans/#Streaming-Only" target="_blank">remote streaming service with RTMP/WebRTC/HLS/DASH</A> (FFMPEG required on web host). Then request streaming provider to enable it. Then access videochat interface as performer. For more details see <a href="https://videowhisper.com/?p=RTMP-Session-Control">session control</a>.',
				'url' => $adminSettings .'server',
			...
			(
				...
				'fix' => 'Go to Performer Dashboard and Go Live using HTML5 or HTML5 app interface. Requires session control to detect the HTML5 stream.',
				'url' => $broadcastPage,
			),
...
?>
    <div id="vwNotice" class="notice notice-success is-dismissible">
        ...
        <?php echo $htmlCode ?>
        <a href="<?php echo $adminPage; ?>setup" >Setup Overview</a>
        | <a href="<?php echo $adminPage; ?>setup&skip=<?php echo $requirement['label']?>">Skip "<?php echo $requirement['title']?>"</a>

        | <a href="<?php echo $adminPage; ?>support" >Support Resources</a>
...
box-shadow: 0 1px 2px rgba(0, 0, 0, 0.2);
	...
	static function requirementRender($label, $view = 'check', $requirement = null)
	{
		...
		case 'overview':
			...
			if (!$isPresent && $requirement['manual']) $htmlButton .=  ' <a class="button" href="' . $adminPage . 'setup&done=' . $requirement['label'] . '">Done</a>';
