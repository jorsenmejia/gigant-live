			...
			if( array_key_exists( 'wirecard', $active_payment_methods ) ) {
				...
				if( $withdrawal_test_mode == 'yes' ) {
					$headers = array(
						...
						'Authorization: Basic ' . base64_encode( $withdrawal_wirecard_test_token . ':' . $withdrawal_wirecard_test_key ),
					);
				} else {
					$headers = array(
						'Content-Type: application/json',
						'Cache-Control: no-cache',
						'Authorization: Basic ' . base64_encode( $withdrawal_wirecard_token . ':' . $withdrawal_wirecard_key ),
					);
				}
